...
from Utils.Get_File_Paths import GetFileList, ChangeToOtherMachine


def convert_vott_csv_to_yolo(vott_df, labeldict=dict(zip(['Cat_Face'], [0, ])), path='', target_name='data_train.txt',
                             abs_path=False):
...
if __name__ == '__main__':
    ...
    multi_df = pd.read_csv(
        'C:/Users/Anton/Documents/Insight/eq/EQ_new/Train_Housing_detector/2/vott-csv-export/Housing_cropping-export'
        '.csv')
    multi_df.drop_duplicates(subset=None, keep='first', inplace=True)
    convert_vott_csv_to_yolo(multi_df, labeldict, path='/home/ubuntu/logohunter/data/houses/',
    ...
    label_names = ['background', 'facade', 'molding', 'cornice', 'pillar', 'window', 'door', 'sill', 'blind', 'balcony',
                   'shop', 'deco']
